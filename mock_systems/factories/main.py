...
col = mongo.get_default_database()["factories_businesses"]
...
app = FastAPI(title="Mock Factories", lifespan=lifespan)
...
@app.get("/health")
def health():
    return {"status": "ok", "system": "factories"}
...
async def _notify(ubid: str, old: dict, new: dict):
    async with httpx.AsyncClient() as client:
        try:
            ...
            print(f"[Factories] webhook failed: {e}")
