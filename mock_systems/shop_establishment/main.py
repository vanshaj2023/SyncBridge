...
col = mongo.get_default_database()["shop_establishment_businesses"]
...
app = FastAPI(title="Mock Shop Establishment", lifespan=lifespan)
...
async def update_business(ubid: str, payload: dict):
    old = await _get(ubid)
    new = {**old, **payload}
    await _set(ubid, new)
    return new
