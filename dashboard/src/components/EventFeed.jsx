...
export default function EventFeed() {
  ...
  const filtered = filter === 'all' ? events
    : filter === 'conflict' ? events.filter(e => e.conflict)
    : events.filter(e => (e.detection_method || 'webhook') === filter)
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '56px 20px' }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-4)' }}>
              {events.length === 0 ? 'Go to Try It Yourself or Scenarios to trigger a sync' : 'Try a different filter'}
            </div>
          </div>
        ) : (
          <>
              ...
              {paged.map((e, i) => {
                ...
                return (
                  <div key={globalIdx} style={{
                    ...
                    {/* Conflict notice */}
                    {e.conflict && (
                      <div style={{ background: 'var(--amber-bg)', borderBottom: '1px solid var(--amber-bd)', padding: '5px 16px', fontSize: 11, color: 'var(--amber)', fontWeight: 500 }}>
                        Conflict resolved — {e.conflict.resolution_policy} — {e.conflict.winner} wins
                      </div>
                    )}

                    {/* Row */}
                    <div style={{ padding: '11px 16px', display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', minWidth: 0 }}
                      ...
                      <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                          ...
                          {(e.targets_failed || []).length > 0 && (
                            <span style={{ fontSize: 11, color: 'var(--red)', flexShrink: 0 }}>· {e.targets_failed.join(', ')} failed</span>
                          )}
                        </div>
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  <button onClick={() => { setPage(p => p - 1); setExpanded(null) }} disabled={page === 0} className="btn btn-secondary" style={{ padding: '5px 12px', fontSize: 12 }}>
                    ← Prev
                  </button>
                  {Array.from({ length: totalPages }, (_, i) => (
                    <button key={i} onClick={() => { setPage(i); setExpanded(null) }} style={{
                      width: 30, height: 30, borderRadius: 6, border: '1px solid',
                      fontSize: 12, cursor: 'pointer', transition: 'all 0.15s',
                      borderColor: page === i ? 'var(--text-2)' : 'var(--border)',
                      background: page === i ? 'var(--text-1)' : 'var(--surface)',
                      color: page === i ? '#fff' : 'var(--text-3)',
                    }}>{i + 1}</button>
                  ))}
                  <button onClick={() => { setPage(p => p + 1); setExpanded(null) }} disabled={page === totalPages - 1} className="btn btn-secondary" style={{ padding: '5px 12px', fontSize: 12 }}>
                    Next →
                  </button>
                </div>
              </div>
